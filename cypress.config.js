    ...
    setupNodeEvents(on, config) {
      ...
      on('task', {
        ...
        updateTestQualityLog({ filePath, newIssues }) {
          ...
          // Calculate summary statistics (simplified to 2 categories)
          const summary = {
            likelyBug: allIssues.filter(i => i && i.category === 'Failed - Likely Bug').length,
            selectorIssue: allIssues.filter(i => i && i.category === 'Selector Issue').length
          };
