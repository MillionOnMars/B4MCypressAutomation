...
export const openProfileTabs = tab => {
  ...
  cy.contains('.MuiTab-colorNeutral', tab, { timeout: DEFAULT_TIMEOUT }).click({
    force: true,
  });
};
...
const navigateToProfileSettings = () => {
  ...
  cy.url().should('include', '/profile', { timeout: DEFAULT_TIMEOUT });
};
...
const RevertUserSettings = user => {
  ...
  //Edit team
  cy.get('input[name="team"]', { timeout: DEFAULT_TIMEOUT }).clear();
  //Edit role
  cy.get('input[name="role"]', { timeout: DEFAULT_TIMEOUT }).clear();
  //Edit phone
  ...
  cy.get('[data-testid="preferred-contact-selectbox"]', {
    ...
    .click({ force: true });
  // Select T-shirt Size
  cy.contains('.profile-data-form-label', 'T-shirt Size:', {
    timeout: DEFAULT_TIMEOUT,
  })
    .parent()
...
const verifyProfileElements = () => {
  ...
  cy.contains('Credits', { timeout: DEFAULT_TIMEOUT }).should('be.visible');
  cy.get('.profile-detail-tab-value', { timeout: DEFAULT_TIMEOUT }).should(
    'be.visible'
  );
  // Verify presence of Storage Used
  cy.contains('Storage Used', { timeout: DEFAULT_TIMEOUT }).should(
    'be.visible'
  );
  ...
  cy.contains('Collection', { timeout: DEFAULT_TIMEOUT }).should('be.visible');
  cy.get('.profile-collection-section-item', {
    timeout: DEFAULT_TIMEOUT,
  }).should('be.visible');
};

const toggleExperimentalFeature = (index, featureName) => {
  // Toggle the feature
...
const verifyQuestMasterFeature = () => {
  navigateToProfileSettings();
  ...
  toggleExperimentalFeature(index, 'Quest Master');
...
const verifArtifactsFeature = () => {
  ...
  const sendMessageAndVerifyArtifact = shouldGenerateArtifact => {
    ...
    if (shouldGenerateArtifact) {
      cy.contains('Generating artifact', { timeout: DEFAULT_TIMEOUT }).should(
        'be.visible'
      );
    } else {
      cy.contains('Generating artifact', { timeout: DEFAULT_TIMEOUT }).should(
  ...
  toggleExperimentalFeature(index, 'Artifacts');
  ...
  static ToggleExperimentalFeatures() {
    describe('Toggle Experimental Features Tests', () => {
      it('Verify Quest Master', () => {
        verifyQuestMasterFeature();
      });
      it('Verify Mementos', () => {
        verifyMementosFeature();
      });
      it('Verify Artifacts', () => {
        verifArtifactsFeature();
      });
    });
  }
}
