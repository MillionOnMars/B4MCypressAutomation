...
const projectTabButtonSelector =
  '[data-testid="project-tab-list"] button[role="tab"]';
...
const createProject = projectName => {
  ...
  cy.xpath('//button[normalize-space()="New Project"]', {
    timeout: DEFAULT_TIMEOUT,
  })
    .should('be.visible')
    .click();
  ...
  cy.get(
    '[data-testid="create-project-form-container"] [data-testid="name-input"]',
    { timeout: DEFAULT_TIMEOUT }
  )
    .eq(0)
    .should('be.visible')
  ...
  cy.get(
    '[data-testid="create-project-form-container"] [data-testid="description-textarea"]',
    { timeout: DEFAULT_TIMEOUT }
  )
    .should('be.visible')
    .type(projectName);
...
const renameProject = (oldName, newName) => {
  ...
  // Type the project name in the input field
  cy.get('[data-testid="project-edit-modal-form"] [data-testid="name-input"]', {
    timeout: DEFAULT_TIMEOUT,
  })
    .eq(0)
    .should('be.visible')
  ...
  cy.xpath('//button[normalize-space()="Update Project"]', {
    timeout: DEFAULT_TIMEOUT,
  })
    .should('be.visible')
    .click();
...
const deleteProject = projectName => {
  ...
  cy.contains('.MuiMenuItem-variantPlain', 'delete', {
    timeout: DEFAULT_TIMEOUT,
    matchCase: false,
  })
    .should('be.visible')
...
const checkAndDeleteProjectIfExists = projectName => {
  ...
  cy.get('body', { timeout: DEFAULT_TIMEOUT }).then($body => {
    if ($body.find(`*:contains("${projectName}")`).length > 0) {
      ...
      cy.log(
        `Project "${projectName}" not found, continuing with test execution`
      );
    }
  });
};
...
const addNotebook = (notebookName, projectName) => {
  ...
  cy.get(projectTabButtonSelector, { timeout: DEFAULT_TIMEOUT })
    .contains('Notebook')
    .should('be.visible')
    .click();
...
const removeNotebook = notebookName => {
  ...
  cy.xpath(`//div[contains(text(),${notebookName})]`, {
    timeout: DEFAULT_TIMEOUT,
  })
    .should('be.visible')
    .click();
  ...
  cy.get('.MuiBox-root.css-i5q2k0 > button').eq(4).should('be.visible').click();

  // Confirm removal in dialog
  cy.get('.MuiButton-sizeMd').contains('Remove').should('be.visible').click();

  // Verify notebook is removed
  cy.contains('Notebook removed successfully', {
    timeout: DEFAULT_TIMEOUT,
  }).should('be.visible');
  cy.contains(notebookName, { timeout: DEFAULT_TIMEOUT }).should('not.exist');
};
...
const uploadFileWithFileBrowser = (promptType, projectName, tabType) => {
  ...
  cy.xpath("//p[normalize-space()='File Browser']", {
    timeout: DEFAULT_TIMEOUT,
  })
    .should('be.visible')
    .click();
  ...
  cy.get('[data-testid="file-browser-list-item"]', { timeout: DEFAULT_TIMEOUT })
    .contains(filename)
    .should('be.visible')
    .click();
...
const clickMembersTab = projectName => {
  ...
  cy.xpath("//button[normalize-space()='Add Members']", {
    timeout: DEFAULT_TIMEOUT,
  }).should('be.visible');
};
...
const addMembers = memberEmail => {
  ...
  cy.contains('Sent an invite to the selected users', {
    timeout: DEFAULT_TIMEOUT,
    matchCase: false,
  }).should('be.visible');
};
...
const handleSystemPrompt = (projectName, action, promptName) => {
  ...
  switch (action) {
    case 'view':
      ...
      cy.contains(promptName, { timeout: DEFAULT_TIMEOUT }).should(
        'be.visible'
      );
    ...
    case 'delete':
      ...
      cy.get('.project-system-prompt-item-menu-item', {
        timeout: DEFAULT_TIMEOUT,
      })
    ...
    case 'edit':
      ...
      cy.get('[data-testid="system-prompt-view-modal"]', {
        timeout: DEFAULT_TIMEOUT,
      }).should('be.visible');
    ...
    default:
      throw new Error(
        `Invalid action: ${action}. Use 'edit', 'delete', or 'view'`
      );
  }
};
...
const logoutUser = () => {
  ...
  cy.get('[data-testid="logout-btn"]').should('exist').click({ force: true });
...
const validateSharedProjects = (projectName, notebook, user) => {
  ...
  cy.contains(`${user}`, { timeout: DEFAULT_TIMEOUT, matchCase: false }).should(
    'be.visible'
  );
