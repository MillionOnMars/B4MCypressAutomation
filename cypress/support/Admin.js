...
const navigateToAdminDashboard = () => {
  ...
  cy.url().should('include', '/admin', { timeout: DEFAULT_TIMEOUT });
};
...
const searchUser = (username, email) => {
  ...
  cy.contains(username, { timeout: DEFAULT_TIMEOUT }).should('be.visible');
  cy.contains(email, { timeout: DEFAULT_TIMEOUT }).should('be.visible');
};
...
const sortname = (username, sortBy) => {
  ...
  cy.get(`[data-testid="${sortTestIdMap[sortBy]}"]`, {
    timeout: DEFAULT_TIMEOUT,
  })
    .should('exist')
    .click({ force: true });
  ...
  cy.get(`[aria-label="${username}"]`, { timeout: DEFAULT_TIMEOUT }).should(
    'be.visible'
  );
};
const CreateUser = userDetails => {
  ...
  //Click Bulk Import
  cy.contains('Bulk Import').should('be.visible').click();
  //Add CSV data
...
const EditUser = user => {
  ...
  cy.xpath("//button[normalize-space()='Profile']")
    ...
    .click();
  // Edit name
  cy.get('input[name="name"]').clear().type(user.newName);
  //Edit team
  cy.get('input[name="team"]').clear().type(user.newTeam);
  //Edit role
  cy.get('input[name="role"]').clear().type(user.newRole);
  //Edit phone
...
const CreateInviteCode = createdby => {
  ...
  //Verify success message
  cy.contains('Registration invite(s) created', {
    timeout: DEFAULT_TIMEOUT,
  }).should('be.visible');
  //Verify created by
  cy.contains(createdby, { timeout: DEFAULT_TIMEOUT }).should('be.visible');
  //Copy invite code
...
const UseInviteCode = (username, email, password, fullname) => {
  ...
  cy.url().should('include', '/register', { timeout: DEFAULT_TIMEOUT });
...
const DeleteInviteCode = username => {
  ...
  //Click Invite Codes tab
  cy.contains('Invite Codes').should('be.visible').click();
  //Click Delete button
  cy.get('[data-testid="delete-invite-code-btn"]')
    ...
    .click();
  //Verify success message
  cy.contains('Registration invite(s) deleted', {
    timeout: DEFAULT_TIMEOUT,
  }).should('be.visible');
};
